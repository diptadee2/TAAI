// Scheduled function (see netlify.toml) — runs once a day and checks any
...
// slipped past manual review entirely. This is the "proactive scan"
// half of the name-moderation system; rename.js's own inline check
// (only while a student is already needs_rename-gated) is the reactive
// half that stops someone dodging an existing flag with a different-but-
// still-bad name.
//
...
// admin/rename-time process resolves that will re-check the new name on
// its own), and (2) their current display_name doesn't match
// name_last_checked (never checked, or checked a since-changed name).
// This second condition is what keeps this cheap on every run after the
// first: once a name has been checked and passed, it's never re-billed
// against the API again unless the student actually changes it.
//
// SCAN_LIMIT caps how many students get checked per run — mainly to
// spread the very first run's backlog (every existing student needs an
// initial check) across several nights instead of one large burst, and
// as a general ceiling against a runaway cost surprise. Every run after
// the backlog clears only ever has a handful of genuinely new/changed
// names to check, well under this cap.
...
export async function handler() {
  ...
  if (error) {
    ...
    // automatically on tomorrow's run regardless.
