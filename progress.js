(function () {
  ...
  var CLIENT_VERSION = '2026-09-24-8';
  ...
  function renderPomodoro() {
    ...
    return '<div class="pomodoro-card fade-in' + (pomo.mode === 'break' ? ' on-break' : '') + '" id="pomo-card" style="' + pomoGradStyle + '">' +
      ...
      // this timer" the way a badge on a dial does, rather than floating
      // in the card's corner with a gap of empty space around it.
      ...
      // inventing a new palette. Defined once here since renderPomodoro()
      // itself only ever renders once per page (the #pomo-ring-gradient-*
  ...
  // This same Today card renders in two different places — the plain
  // checklist page, and again inside Focus Mode (.pomo-today-row's
  // sibling, between the timer/leaderboard row and the weekly board).
  // The "Activity by hour" chart + busy meter (pomoHourlyActivityHtml)
  // was dropped from the plain-checklist rendering on direct request
  // ("just the daily task without the extra shenanigan" — that request
  // was about a screenshot of the checklist page specifically), but a
  // follow-up report ("why did you take the busy meter and bar graph
  ...
  function renderTodayCard(day, missedBeforeCount, showHourly) {
    ...
    if (!showHourly) {
      return '<div class="today-card fade-in">' + left + '</div>';
    }
    var html = '<div class="today-card-split">';
  ...
  // comparison. That earlier version meant two viewers checking at
  ...
  // All-time cumulative-effort milestone badges, shown beside a name on
  // the weekly top-20 board — a different axis from the streak tiers
  // above (consecutive days) and the 🥇🥈🥉 rank medals (this week's
  // placement): total hours ever logged, regardless of streak or rank.
  // Deliberately non-medal emojis (no 🥇🏅🏆-style icons — those already
  // mean "ranking" elsewhere on this same row) escalating from "getting
  // serious" to "elite," on an ascending-difficulty spread in the same
  // spirit as STREAK_TIERS. Real production data at ship time tops out
  ...
  // Badges were tried inline after the name, then on their own row below
  // it — both read as cluttering an already info-dense row (rank, move
  // arrow, live dot, name, streak balls, status, timer, minutes), per
  // direct feedback both times ("too congested," then "too much going
  // on"). Pulled off the row entirely — effortBadgesHtml/EFFORT_TIERS
  // are still very much used, just by the two places that already
  // explain a badge without adding row clutter: the all-time-hours
  // hover tooltip (showAllTimeTooltip) and the Name column's "?" legend
  // (effortLegendHtml). This function is now a thin pass-through kept
  // only so the two call sites below don't need reverting separately.
  function leaderboardNameCellHtml(nameInnerHtml, dataAttr) {
    return '<span class="leaderboard-name"' + dataAttr + '>' + nameInnerHtml + '</span>';
  }
  ...
  function renderLeaderboardRows(animate) {
    ...
    var rows = state.leaderboard.map(function (r, i) {
      ...
      return '<div class="leaderboard-row' + (i < 3 ? ' leaderboard-row--top' : '') + (r.is_me ? ' leaderboard-row--me' : '') + (r.is_live ? ' leaderboard-row--live' : '') + leaderboardRowEnterAttrs(animate, i) + '">' +
        ...
        leaderboardNameCellHtml(liveDotHtml(r.is_live) + escapeHtml(r.display_name) + (r.is_me ? ' <span class="leaderboard-you">You</span>' : ''), allTimeTitleAttr(r.all_time_minutes)) +
    ...
    if (state.viewerRank) {
      rows += '<div class="leaderboard-gap">···</div>' +
        ...
        leaderboardNameCellHtml(liveDotHtml(state.viewerRank.is_live) + 'You', allTimeTitleAttr(state.viewerRank.all_time_minutes)) +
